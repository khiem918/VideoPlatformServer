...
export class NotificationService {
    ...
    async sendNotification(userId: string, notification_subject: string, payload: string, type: string) {
        const res = await this.NotificationRepository.createNotification(userId, notification_subject, payload, type);
        const targetUserIds = await this.NotificationRepository.getSubscribedChannelUserIds(userId);
        await this.RedisNotifyService.publishToRedisStream(STREAM_KEY, userId, payload, res.id, type, notification_subject, targetUserIds.map(u => u.userId).concat([userId]));
    }
