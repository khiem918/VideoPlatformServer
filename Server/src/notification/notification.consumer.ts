...
export class NotificationConsumer implements OnModuleInit, OnModuleDestroy {
    ...
    private async startConsuming() {
        ...
        while (this.running) {
            try {
                const messages = await this.redisNotifyService.consumeFromRedisStream(STREAM_KEY, GROUP_NAME, CONSUMER_NAME, 10, 2000);
                ...
                this.logger.error('Error occurred while consuming messages from Redis stream :', error);
    ...
    private async handleMessage(id: string, fields: string[]) {
        try {
            ...
            const targetUserIds = JSON.parse(data['targetUserIds'] || '[]') as string[];

            this.logger.log(`Processing notification for user ${userId} with payload: ${payload} and type: ${type} and targetUserIds: ${targetUserIds.join(',')}`);
            ...
            await this.redisNotifyService.acknowledgeMessage(STREAM_KEY, GROUP_NAME, id);
        } catch (error) {
            this.logger.error(`Error processing notification for user ${id}: ${error.message}`);
        }
    }
