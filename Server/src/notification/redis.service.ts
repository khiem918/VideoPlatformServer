...
export class RedisNotifyService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor() {
        ...
        this.sub.on('connect', () =>
            this.logger.log('sub connecting...'),
        );

        this.pub.on('ready', () =>
            this.logger.log('pub connected and ready'),
        );
        this.sub.on('ready', () =>
            this.logger.log('sub connected and ready'),
        );
    ...
    async publishToRedisStream(
        ...
        try {
            await this.pub.xadd(key, '*', 'userId', userId, 'payload', payload, 'id', id, 'type', type, 'subject', subject, 'targetUserIds', JSON.stringify(targetUserIds));
            this.logger.log(`Published message to Redis stream ${key} for user ${userId}`);
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to publish message to Redis stream: ${message}`);
        }
    }
    ...
    async ensureConsumerGroup(key: string, groupName: string): Promise<void> {
        try {
            ...
            if (error instanceof Error && error.message.includes('BUSYGROUP')) {
                this.logger.warn(`Consumer group ${groupName} already exists for stream ${key}`);
            } else {
                const message = error instanceof Error ? error.message : 'Unknown error';
    ...
    async consumeFromRedisStream(key: string, groupName: string, consumerName: string, count: number, block: number): Promise<any[]> {
        try {
            ...
            const message = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to consume messages from Redis stream: ${message}`);
            return [];
        }
    }

    async acknowledgeMessage(key: string, groupName: string, id: string): Promise<void> {
        try {
            await this.sub.xack(key, groupName, id);
            this.logger.log(`Acknowledged message ${id} in stream ${key} for group ${groupName}`);
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unknown error';
